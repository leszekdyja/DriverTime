...
function getActivityStartOdometer(activity: TachographActivity) {
    return activity.startOdometerKm ?? activity.odometerStart ?? null;
}

function getActivityEndOdometer(activity: TachographActivity) {
    return activity.endOdometerKm ?? activity.odometerEnd ?? null;
}

function getActivityDistanceKm(activity: TachographActivity) {
    if (activity.distanceKm !== null && activity.distanceKm !== undefined && Number.isFinite(activity.distanceKm)) {
        return activity.distanceKm;
    }

    const startOdometer = getActivityStartOdometer(activity);
    const endOdometer = getActivityEndOdometer(activity);
...
function buildSegmentsForDay(activities: TachographActivity[], day: string, countryEntries: TachographCountryEntry[], vehicleUses: TachographVehicleUse[], violations: TachographViolation[]): Segment[] {
    ...
    return activities
        .map((activity): Segment | null => {
            ...
            const distanceKm = type === "DRIVING" && isWholeActivitySegment ? getActivityDistanceKm(activity) : null;
            const distanceSource = distanceKm !== null && distanceKm !== undefined ? "activity" : null;
            const startOdometerKm = type === "DRIVING" ? getActivityStartOdometer(activity) : null;
            const endOdometerKm = type === "DRIVING" ? getActivityEndOdometer(activity) : null;
            const averageSpeedKmh = type === "DRIVING" && distanceKm !== null && distanceKm !== undefined ? activity.averageSpeedKmh ?? (seconds > 0 ? distanceKm / (seconds / 3600) : null) : null;
...
function buildMarkersForDay(day: string, countryEntries: TachographCountryEntry[], vehicleUses: TachographVehicleUse[], violations: TachographViolation[], cardReadings: TachographCardReading[]): Marker[] {
    ...
    for (const vehicleUse of vehicleUses) {
        ...
        const endTime = getVehicleUseEnd(vehicleUse);
        markers.push({
            id: vehicleUse.id ?? `vehicle-${registration}-${timestamp.toISOString()}`,
            kind: "vehicle",
            left: clampPercent(percentFor(timestamp, start)),
            label: `V ${formatVehicleMarkerLabel(registration)}`,
            title: `Pojazd: ${registration}\nStart: ${dateTimeFormatter.format(timestamp)}${endTime ? `\nKoniec: ${dateTimeFormatter.format(endTime)}` : ""}`,
        });
    }
